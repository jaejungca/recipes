---
title: "Week 2: Data Splitting and Preprocessing with recipes"
...
p1 <- ggplot(df_raw, aes(x = income)) +
  annotate("rect",
    xmin = gsd_low, xmax = gsd_high, ymin = -Inf, ymax = Inf,
    fill = "#97C459", alpha = 0.15) +
  geom_histogram(bins = 50, fill = "#85B7EB", color = "#378ADD", linewidth = 0.3) +
  geom_vline(aes(xintercept = median_raw, color = "Median"), linewidth = 1) +
  geom_vline(aes(xintercept = mean_raw,   color = "Mean"),   linewidth = 1, linetype = "dashed") +
  annotate("text", x = median_raw - 1500, y = Inf,
    label = "median\n$49K", color = "#E24B4A", vjust = 1.3, hjust = 1, size = 3) +
  annotate("text", x = mean_raw + 1500, y = Inf,
    label = "mean\n$59K",   color = "#BA7517", vjust = 1.3, hjust = 0, size = 3) +
...
p2 <- ggplot(df_log, aes(x = log_income)) +
  annotate("rect",
    xmin = log_sd_low, xmax = log_sd_high, ymin = -Inf, ymax = Inf,
    fill = "#97C459", alpha = 0.15) +
  geom_histogram(bins = 40, fill = "#85B7EB", color = "#378ADD", linewidth = 0.3) +
  geom_vline(aes(xintercept = log_median, color = "Median"), linewidth = 1) +
  geom_vline(aes(xintercept = log_mean,   color = "Mean"),   linewidth = 1, linetype = "dashed") +
  annotate("text", x = log_median - 0.03, y = Inf,
    label = "median\n10.8",  color = "#E24B4A", vjust = 1.3, hjust = 1, size = 3) +
  annotate("text", x = log_mean + 0.03, y = Inf,
    label = "mean\n10.98",   color = "#BA7517", vjust = 1.3, hjust = 0, size = 3) +
  scale_color_manual(values = c("Median" = "#E24B4A", "Mean" = "#BA7517")) +
...
## 4.2 Deep Dive: Each Step

### 4.2.1 Imputation (`step_impute_median`)
...
### 4.2.2 Log Transformation (`step_log`)
...
### 4.2.3 Normalization (`step_normalize`)
...
### 4.2.4 Dummy Variables (`step_dummy`)
...
# 6 Connecting the Recipe to a Model Workflow
...
head(preds |> select(customer_id, responded, .pred_class, .pred_yes))
```
...
# 7 Inspecting and Debugging a Recipe
...
```{r tidy-step}
# Inspect normalization statistics (step 4 = step_normalize)
tidy(mail_prep, number = 4) |> arrange(terms)
```

```{r tidy-impute}
# Inspect imputation values (step 2 = step_impute_median)
tidy(mail_prep, number = 2)
```
